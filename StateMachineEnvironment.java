 ...
 * 
 * An environment in which the agent can make moves in an attempt to change it's
 * state and reach the goal state. The agent will make moves and if a chosen
 * move brings the agent to a new state, it's sensor will be true, if not it
 * will be false. The agent is also able to sense if it has reached the desired
 * goal state.
 * 
...
public class StateMachineEnvironment {
    ...
    private void generateStateMachine() {
        ...
        // numerical designation. We index into the array using the number of
        // the
        ...
        // Initialize all the values to -1 so we can tell if there's a
        // transition
        // there or not (since 0 is a valid state to transition to, and the
        // array
        ...
        for (int i = 0; i < NUM_STATES; i++) {
            ...
            for (int j = 0; j < NUM_TRANSITIONS; j++) {
                ...
                // if there is already a transition for that character, generate
                // a new character to transition on
    ...
    /**
     * A method which iterates through and prints out the two-dimension array
     * that represents the state machine
     */
    ...
    /**
     * A method which takes in a move from the agent and updates the current
     * state and the agent's sensors if needed.
     * 
     * @param move
     *            The move the agent is making
     * @return The agent's updated sensors
     */
    ...
    /**
     * A helper method which determines a given letter's location in the
     * alphabet for the tick method
     * 
     * @param letter
     *            The letter who's index we wish to find
     * @return The index of the given letter (or -1 if the letter was not found)
     */
     ...
     * @return The index into the alphabet array of the character fromState
     *         reads to transition to toState, or -1 if no such character exists
     */
    ...
    public void findShortestPaths() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < NUM_STATES; i++) {
                ...
                // character from state i to the current state added to the
                // front
    ...
    /**
     * A helper method that prints the shortest path from each state to the
     * goal.
     */
