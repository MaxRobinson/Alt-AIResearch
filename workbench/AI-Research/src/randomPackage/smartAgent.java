...
public class smartAgent extends Agent{
	...
	public void initTransTable(){
		//Add's a buffer row to the transition Table since we do not have a state with ID = 0.
		...
		//adds the last state, The goal state, To the transition table. (This was not happening previously)
	...
	public ListAndBool analyzeMove(){
		...
		//Not necessarily for testing anymore  don't need if any more?? probs not.
	...
	public void addPathToTransTable(ListAndBool listOfNewEpisodes){
		...
		int indexOfRow = this.currentState;   //Should double check to make sure that this is actually the state that is conjectured to be the same or diff.  
		int indexOfChar = findIndex(newEpisodes.get(0).command); //this will get the first move from conjectured state to the first episode in listOfNewEpisodes
